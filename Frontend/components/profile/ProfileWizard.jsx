...
export default function ProfileWizard({ initialUser, onSaved, showSuccessScreen = true }) {
  ...
  const [photoPreview, setPhotoPreview] = useState(initialUser?.avatar_url || null);
  ...
  const [professionRole, setProfessionRole] = useState(initialUser?.profession_role || "");
  const [isFreelance, setIsFreelance] = useState(initialUser?.workplace === "Freelance");
  const [workplace, setWorkplace] = useState(
    initialUser?.workplace === "Freelance" ? "" : initialUser?.workplace || "",
  );
  const [mediaType, setMediaType] = useState(initialUser?.media_type || []);
  const [coverageAreas, setCoverageAreas] = useState(initialUser?.coverage_areas || []);
  const [contentTypes, setContentTypes] = useState(initialUser?.content_types || []);

  const [aiFamiliarity, setAiFamiliarity] = useState(initialUser?.ai_familiarity || "");
  const [aiTools, setAiTools] = useState(initialUser?.ai_tools || []);
  const [timeConsumingTask, setTimeConsumingTask] = useState(initialUser?.time_consuming_task || "");
  const [periodistaiaWishes, setPeriodistaiaWishes] = useState(initialUser?.periodistaia_wishes || "");
  const [helpPreference, setHelpPreference] = useState(initialUser?.help_preference || "");
  ...
  async function handleSave() {
    ...
    try {
      ...
      await refreshUser();
      if (showSuccessScreen) setSuccess(true);
      await onSaved?.();
    } catch (err) {
